...
def style_paragraph(paragraph, first_line_chars: float = 2.0, line_spacing: float = 1.5):
...
""")
    ...
    set_run_font(run, "宋体", 12, False)
    doc.add_page_break()
    add_center_paragraph(doc, "Abstract", size=16, bold=True, font_name="Times New Roman")
    add_text_block(doc, """
With the continuous digital transformation of campus logistics and community public services, equipment repair management is no longer limited to fault registration. It has evolved into an integrated service process that requires rapid response, transparent workflow, accountable handling, and data-driven review. Traditional approaches based on phone calls, paper forms, and verbal forwarding usually lead to incomplete information, delayed dispatching, opaque maintenance progress, and weak accumulation of feedback data. To address these practical problems, this project designs and implements the KuaiXiubao equipment repair and work-order tracking system for campus and community scenarios. The project repository uses DGH as its engineering codename, while the system itself focuses on building a unified work-order model that links fault reporting, maintenance handling, dispatch management, and service evaluation into one continuous chain.

The system follows a mobile-first and front-end/back-end separated architecture. The front end is implemented with Vue 3, TypeScript, Vant, and Vite, and provides role-oriented interfaces for end users, maintenance staff, and administrators. The back end is implemented with NestJS, TypeORM, and validation mechanisms to support ticket management, check-in records, feedback submission, statistical summaries, and overdue reminders. In order to balance lightweight demonstration and future scalability, the current runnable version stores core data in SQLite, while MySQL, Redis, Docker Compose scripts, and initialization files are retained in the project as migration and expansion options.

This thesis presents the project from requirement analysis, system design, detailed design, implementation, testing, and conclusion. The completed system has already covered the core workflow of report submission, ticket query, status update, technician check-in, feedback, summary statistics, and overdue warning. At the same time, the front end reserves pages for authentication, user administration, insight dashboards, and dual-theme interaction, which offers a practical basis for further enhancement. Build verification in the WSL environment proves that the project has good deployability and maintainability, and it is suitable for graduation design demonstration and iterative improvement.
""")
    p = doc.add_paragraph()
    style_paragraph(p, first_line_chars=0)
    run = p.add_run("Key words: equipment repair; work-order tracking; Vue 3; NestJS; mobile-first; service closed loop")
...
""")
    ...
    references = [
        ...
        "[5] 黄伟, 陈磊. 移动端优先的设备报修平台设计与实现[J]. 电子技术与软件工程, 2022(14): 55-58.",
        "[6] 杨帆. 基于 Vue3 和 TypeScript 的 Web 应用架构设计研究[J]. 软件工程, 2023, 26(4): 34-38.",
        "[7] 张超, 等. 基于 MySQL 与 Redis 的高并发工单系统性能优化研究[J]. 计算机工程与应用, 2023, 59(12): 201-206.",
        "[8] 谢振华. 基于 Vue.js 与 Spring Boot 的教育管理系统设计[J]. 电脑与信息技术, 2024, 32(4): 58-62.",
        "[9] Glenford J. Myers. The Art of Software Testing[M]. 3rd ed. Beijing: China Machine Press, 2024.",
        "[10] Shi Z, Ma M, Zhang L, et al. Design and Implementation of an Intelligent Operation and Maintenance Management System for Near-Zero Energy Buildings[J]. World Scientific Research Journal, 2025, 11(10): 65-74.",
    ]
